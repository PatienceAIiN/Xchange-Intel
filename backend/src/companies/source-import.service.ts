...
export class SourceImportService implements OnModuleInit {
  ...
  stats = { ran: false, imported: 0, skipped: 0, total: 0, at: null as string | null };
  ...
  async importAll() {
    ...
    try {
      ...
      this.stats = { ran: true, imported: added, skipped, total: rows.length, at: new Date().toISOString() };
      this.log.log(`Import complete: +${added} new, ${skipped} duplicates/empty skipped`);
    } finally {
